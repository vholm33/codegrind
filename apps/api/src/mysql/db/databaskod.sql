...
CREATE TABLE IF NOT EXISTS users (
   ...
   createdAt DATETIME(3) DEFAULT CURRENT_TIMESTAMP(3) NULL
);
...
CREATE TABLE IF NOT EXISTS codeQuestions(
    ...
    categoryId INT,
    codeQuestion VARCHAR(500) NOT NULL,
    codeAnswer VARCHAR(500) NOT NULL,
...
VALUES
    ...
    (3, 'Gör en type "Person", med egenskaper "name" och "age" med rätt datatyper.', 'type Person = { name: string; age: number; }'),
...
-- 1. Kopplas till hela quiz sessionen
...
CREATE TABLE IF NOT EXISTS quizAnswers(
    id INT PRIMARY KEY AUTO_INCREMENT,
    sessionId INT NOT NULL,             -- Användarens quiz-session
    questionId INT NOT NULL,            -- Användarens fråga i sessionen
    -- questionCategory INT NOT NULL,   --? Ska man ha med kategorin
    attempts INT NOT NULL,              -- Användarens försök per fråga
    points INT NOT NULL,                -- Användarens poäng per fråga
    isCorrect BOOLEAN NOT NULL,         -- Lyckades användaren få rätt?

    FOREIGN KEY (sessionId) REFERENCES quizSessions(id) ON DELETE CASCADE,
